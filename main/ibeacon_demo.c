...
void app_main(void)
{
    ...
    if (status == ESP_OK){
        ...
        strcpy(test_ble_name,"V9");
        int error = esp_ble_gap_set_device_name(test_ble_name);
        if (error != ESP_OK) {
            ESP_LOGE(DEMO_TAG, "esp_ble_gap_set_device_name: rc=%d", error);
            //return;
        }else{
            ESP_LOGI(DEMO_TAG, "Device name set to %s", test_ble_name);
        }
    }
    else {
